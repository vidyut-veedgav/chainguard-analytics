...
Preprocesses the three source CSVs (ALE, UEM, SIH) into a standard frame indexed by
...
performed here, just preprocessing
    ...
    pp = build_feature_frame("data/raw/account_lifecycle_events.csv",
...
def build_feature_frame(
    ...
    """Raw CSV paths -> one-row-per-account feature frame.

    Loads each source CSV (with the appropriate parse_dates), then applies all
    deterministic transforms. Returns a preprocessed dataframe with all derived/encoded
    ...
    sih_df = pd.read_csv(sih_path, parse_dates=list(SIH_TIMEZONE_COLUMN_MAP))
